...
public class FlawlessFindAndTargetEnemiesShortRange
    extends Script
{
    ...
    public void onTick() {
        ...
        if (getBoolEquals("IgnoreEnemies", Yesno.No)) {
            ...
            if (receivedMessage("FocusThisGuy")) {
                ...
                // Any Enemies?
                if (isCharacterInArea(EnumSet.of(TargetReceiveGroups.PLAYERS, TargetReceiveGroups.CREEPS), EnumSet.of(Teams.ENEMY_TEAM), "", Yesno.No, Yesno.No, Yesno.Yes, CharactervaluesCheckable.Health, Valuecompare.GreaterOrEqual, 0.0D, "", 0.0D, 0.0D, 1.5D, 1.5D, Yesno.Yes)) {
                    // Priority 1: Summoner
                    selectTarget(EnumSet.of(TargetGroups.PLAYERS), EnumSet.of(Teams.ENEMY_TEAM), "Summoner", Yesno.No, Yesno.No, 0.0D, 0.0D, 0.7D, 0.7D, Valuecompare2 .Lowest, 0.0D, DistanceCheck.NONE, Yesno.Yes, Yesno.No);
                    if (hasTarget(Yesno.No)) {
                        // Priority 2: Players
                        selectTarget(EnumSet.of(TargetGroups.PLAYERS), EnumSet.of(Teams.ENEMY_TEAM), "", Yesno.No, Yesno.No, 0.0D, 0.0D, 1.5D, 1.5D, Valuecompare2 .Lowest, 0.0D, DistanceCheck.NONE, Yesno.Yes, Yesno.No);
