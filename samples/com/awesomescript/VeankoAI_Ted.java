    ...
    extends Script
{
    ...
    public void onTick() {
        ...
        if (checkCounter("state", "1", Valuecompare.Equal)||checkCounter("state", "6", Valuecompare.Equal)) {
            if (checkCharacterValue(CharactervaluesCheckable.AISkill, 0.0D, Valuecompare.GreaterOrEqual, Targetself.Self)) {
                setBool("shoot", Flagtoggle.No);
            }
        }
        ...
        if (getBoolEquals("false", Yesno.No)) {
            ...
            if (checkCounter("state", "1", Valuecompare.Equal)) {
                ...
                if (isUpgradeEnabled(Yesno.Yes, "CommandoSwitchStance")) {
                    if (isUpgradeEnabled(Yesno.Yes, "CommandoAirstrikeHealthpack1")) {
                        if (isInArea(Yesno.Yes, EnumSet.of(CollisonGroups.PLAYERS), Yesno.No, Yesno.Yes, EnumSet.of(Teams.ENEMY_TEAM), "", 0.0D, 0.0D, 1.0D, 1.0D, Yesno.No, Yesno.No)) {
                            selectTarget(EnumSet.of(TargetGroups.PLAYERS), EnumSet.of(Teams.OWN_TEAM), "Commando", Yesno.No, Yesno.No, 0.0D, 0.0D, 0.1D, 0.1D, Valuecompare2 .LessOrEqual, 35.0D, DistanceCheck.NONE, Yesno.No, Yesno.No);
                            sequence6();
                        }
                    }
                }
            ...
            if (checkCounter("state", "2", Valuecompare.Equal)) {
                ...
                executeBehaviourTree("SuperTedFindAndTargetEnemiesShortRange");
                ...
                // You're kinda low
                if (checkCharacterValue(CharactervaluesCheckable.HealthPercentage, 35.0D, Valuecompare.Less, Targetself.Self)) {
            ...
            if (checkCounter("state", "3", Valuecompare.Equal)) {
                ...
                if (checkCharacterValue(CharactervaluesCheckable.HealthPercentage, 75.0D, Valuecompare.Less, Targetself.Self)) {
                    executeBehaviourTree("SuperTedFindHealthPack");
                }
                if (getBoolEquals("Outnumbered", Yesno.Yes)) {
                    adjustCounter("setState", "11", Valueadjust.Set);
                }
            ...
            if (checkCounter("state", "4", Valuecompare.Equal)) {
                ...
                // Are you facetanking?
                if (isTurretInArea(EnumSet.of(Teams.ENEMY_TEAM), Valuecompare.GreaterOrEqual, 25.0D, 0.1D, 0.0D, 0.1D, 0.1D, Yesno.No)) {
                    // stop it nub
                    removeTarget();
                }
                // You're kinda low
                ...
                if (getBoolEquals("Combat", Yesno.No)) {
                    ...
                    if (isTargetInArea(Yesno.Yes, 0.0D, 0.0D, 3.0D, 3.0D, Yesno.Yes, Yesno.No)) {
                        ...
                        if (isTargetInArea(Yesno.No, 0.0D, 0.0D, 0.55D, 0.55D, Yesno.Yes, Yesno.No)) {
                            ...
                            executeBehaviourTree("SuperTedCheckIfEnemyTooClose");
                            if (getBoolEquals("EnemyIsTooClose", Yesno.Yes)||getBoolEquals("backOff", Yesno.Yes)) {
                                ...
                                executeBehaviourTree("SuperTedCheckIfEnemyInCombatRange");
            ...
            if (checkCounter("state", "6", Valuecompare.Equal)) {
                ...
                if (isTurretInArea(EnumSet.of(Teams.ENEMY_TEAM), Valuecompare.GreaterOrEqual, 25.0D, 0.1D, 0.0D, 0.1D, 0.1D, Yesno.No)) {
                    if (directionToEnemyBase(Forwardbackward.FORWARD)) {
                        pressStick(Directions.LEFT, 0.3D);
                        removeTarget();
                    } else {
                        pressStick(Directions.RIGHT, 0.3D);
                        removeTarget();
                    }
                }
                setBool("UseRetreatingSkills", Flagtoggle.Yes);
