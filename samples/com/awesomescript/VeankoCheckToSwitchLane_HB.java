...
public class VeankoCheckToSwitchLane_HB
    extends Script
{
    ...
    public void onTick() {
        if (getBoolEquals("LetsSwitchLane", Yesno.No)) {
            if (getBoolEquals("CheckToSwitchLane", Yesno.Yes)) {
                ...
                // Allys not close?
                if (isCharacterInArea(EnumSet.of(TargetReceiveGroups.PLAYERS), EnumSet.of(Teams.OWN_TEAM), "", Yesno.No, Yesno.No, Yesno.Yes, CharactervaluesCheckable.Health, Valuecompare.GreaterOrEqual, 0.0D, "", 0.0D, 0.0D, 2.0D, 1.8D, Yesno.No)) {
                } else {
                    log("CHECK TO SWITCH", "", "");
